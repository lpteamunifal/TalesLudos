    ...
    public function update(Request $request){
    	...
    	$validator = Validator::make($request->all(),
    		[
    			...
    			'last_name' => 'required|max :100|string',
    			'email' => 'required|email|max: 255|unique:users',
    			'old_password' => 'required|alpha_num|in:'.$oldpassqord,
    			'new_password' => 'required|alpha_num|min: 6',
    			're_password' => 'required|alpha_num|in:'.$newpassword,
    		],
    		[
        		'required' => 'O campo :attribute é obrigatório',
        		'unique' => 'O email já esta vinculado a outra conta',
        		'email' => 'Email inválido',
        		'min' => 'A senha deve conter no minímo 6 letras',
        		'in' => 'Confirmação de senha ou senha antiga não confere',
        		'max' => 'O campo :attribute pode conter no maxímo :max caracteres'
		    ], 
		    [
		        'name'      => 'Nome',
    	...
    	if($validator->fails()){
    		return redirect()->back()->withErrors($validator->errors(), '/contact')->withInput(Input::except('password','re_password'));
    	}else{
    		$user = new User();
